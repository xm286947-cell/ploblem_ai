...
case "$ACTUAL_PLATFORM" in
  linux*) ACTUAL_PLATFORM="linux" ;;
  darwin*) ACTUAL_PLATFORM="darwin" ;;
esac
...
if [ ! -f "app/config/model.yaml" ]; then
...
if [ ! -x "$VENV_PY" ]; then
  ...
  "$VENV_PY" -m pip install --disable-pip-version-check -r "$SCRIPT_DIR/app/requirements.txt"
fi

mkdir -p "$SCRIPT_DIR/data/runtime"

cd "$SCRIPT_DIR/app"
"$VENV_PY" "scripts/major_repeat_test_rc1.py" smoke

echo "Starting existing Quality Platform Web at http://127.0.0.1:8080"
exec "$VENV_PY" "scripts/major_repeat_test_rc1.py" serve "$@"
