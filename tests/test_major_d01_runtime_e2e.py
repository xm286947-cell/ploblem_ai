...
def _setup_case(
    ...
    *,
    ...
    case = repo.create_case(title, "G1", "SOFTWARE")
    intake = repo.ingest_file(
        case["case_id"],
        _docx(tmp_path / filename),
    )
    version_id = intake["version_id"]
    parsed = parse_document(repo.attachment_path(version_id))
    repo.save_parse_result(version_id, parsed)
...
def test_major_d01_configured_runtime_provider_end_to_end(tmp_path: Path) -> None:
    with running_server() as (host, port):
        ...
        case, version_id, event = _setup_case(
            repo,
            tmp_path,
            title="D01真实链路模拟验收",
            itr="ITR2026092101",
