...
def test_storage_runtime_acceptance_truncated_then_runtime_retry_completes(tmp_path):
...
def test_storage_runtime_acceptance_retry_budget_exhaustion_blocks_commit(tmp_path):
    store = SqliteTaskStore(tmp_path / "runtime.db")
    runtime = LightweightExecutionEngine(store)
