...
class ValidationItemStatus(str, Enum):
    PENDING = "PENDING"
    CONFIRMED = "CONFIRMED"
    NOT_APPLICABLE = "NOT_APPLICABLE"
...
class ValidationItem(ContractModel):
    ...
    description: str
    status: ValidationItemStatus = ValidationItemStatus.PENDING
    title: str | None = None
...
class InMemoryFormalKnowledgeReleaseConsumer:
    """Test-only seam; production consumer binding is deferred to Integration Gate."""
...
class SoftwareImpactEngine:
    ...
    def _result(request, status, *, impacts=None, knowledge_refs=None, evidence_refs=None, missing=None, confidence=None, trace=None):
        ...
        replay = {"request_id": request.request_id, "schema_version": request.schema_version, "usage_context": request.usage_context, "requested_topics": request.requested_topics, "knowledge_refs": knowledge_refs, "evidence_refs": evidence_refs, **(trace or {})}
        digest = sha256(json.dumps({"request_id": request.request_id, "device_id": request.device_id, "replay": replay}, sort_keys=True, default=str).encode()).hexdigest()[:16]
        return SoftwareImpactAnalysisResult(request_id=request.request_id, analysis_id=f"IMPACT-{digest}", device_id=request.device_id, usage_context=request.usage_context, requested_topics=request.requested_topics, status=status, impacts=impacts or [], knowledge_refs=knowledge_refs, evidence_refs=evidence_refs, missing_information=missing or [], confidence_basis=confidence or [], replay_trace=replay)
    ...
    def analyze(self, request: SoftwareImpactAnalysisRequest) -> SoftwareImpactAnalysisResult:
        if request.usage_context.get("not_applicable") is True:
            return self._result(request, SoftwareImpactAnalysisStatus.NOT_APPLICABLE, missing=["REQUEST_MARKED_NOT_APPLICABLE"])
        ...
        impacts = []; fact_gaps = []; knowledge_gaps = []
        for release in valid_releases:
            missing_triggers = [name for name in release.trigger_fact_names if name not in facts_by_name]
            if missing_triggers:
                fact_gaps.extend(f"{release.release_id}:TRIGGER_FACT_REQUIRED:{name}" for name in missing_triggers); continue
            if not release.technical_meaning or not release.software_impact:
                knowledge_gaps.append(f"{release.release_id}:FORMAL_IMPACT_CONTENT_REQUIRED"); continue
            if not release.trigger_fact_names:
                fact_gaps.append(f"{release.release_id}:FACT_SUPPORT_REQUIRED"); continue
            ...
            if not validation: knowledge_gaps.append(f"{release.release_id}:VALIDATION_ITEMS_REQUIRED"); continue
        ...
        if not impacts:
            status = SoftwareImpactAnalysisStatus.INSUFFICIENT_KNOWLEDGE if knowledge_gaps else SoftwareImpactAnalysisStatus.INSUFFICIENT_FACT
            return self._result(request, status, knowledge_refs=[r.release_id for r in valid_releases], evidence_refs=[ref for r in valid_releases for ref in r.evidence_refs], missing=sorted(set(knowledge_gaps + fact_gaps or ["NO_TRIGGERED_FORMAL_IMPACT"])))
        return self._result(request, SoftwareImpactAnalysisStatus.EVIDENCED, impacts=impacts, knowledge_refs=[ref for item in impacts for ref in item.knowledge_refs], evidence_refs=[ref for item in impacts for ref in item.evidence_refs], missing=sorted(set(knowledge_gaps + fact_gaps)), confidence=["FACT_ANALYSIS_SEPARATION", "FORMAL_KNOWLEDGE_ONLY", "EVIDENCE_TRACEABLE"], trace={"impact_ids": [item.impact_id for item in impacts]})
