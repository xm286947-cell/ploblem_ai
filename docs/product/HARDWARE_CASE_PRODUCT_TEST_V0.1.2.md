...
Release claim: **NO**  
Package: `HARDWARE_CASE_PRODUCT_TEST_V0.1.2_<commit>.zip`

V0.1.2 is the startup-closure revision. It keeps the V0.1.1 Agent/Runtime
configuration work and fixes a real Windows startup defect found during company
testing.

The defect was not a missing pip dependency. The old start script invoked the
repository-wide main.py, whose top-level imports pulled unrelated Repeat Case
builder modules and then failed because the trimmed Hardware Case package did
not contain the repository's internal parser package.

V0.1.2 therefore starts the same create_p0_app() through a Hardware Case
launcher instead of the repository-wide CLI. It does not create a second Web
server or a second API implementation.
...
2. executes scripts/hardware_case_web_start.py;
3. initializes the same Quality Capability P0/P1 database;
4. starts the same create_p0_app() application on port 8080;
5. opens the browser.

The package intentionally does not use repository-wide main.py for this start
path. Do not install a third-party package named parser if a historical V0.1.1
package shows ModuleNotFoundError: parser; replace that package with V0.1.2.
...
V0.1.2 preserves the frozen P07 product rules:
...
P07_FRONTEND=PASS
RUNTIME_PACKAGE_ASSETS=PASS
STARTUP_PRECHECK=PASS
PACKAGE_STATUS=READY_FOR_INTERNAL_TEST
RELEASE_CLAIM=NO
```

A second startup gate also runs directly from the built package:
