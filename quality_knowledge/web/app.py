...
def create_app(db_path):
    ...
    def quality_scenario_generate_page(request: Request, product_code: str = '', start_month: str = '', end_month: str = '', preview: int = 0, job_id: str = '', source: str = 'cs_itr', ipmt: str = '', spdt: str = '', product_model: str = '', year: str = ''):
        ...
        filters={'product_code':product_code,'start_month':start_month,'end_month':end_month,'source':source,'ipmt':ipmt,'spdt':spdt,'product_model':product_model,'year':year,'group_ids':selected_groups}
    ...
    def quality_scenario_generate(request: Request, product_code: str = Form(...), start_month: str = Form(''), end_month: str = Form(''), selected_ids: list[str] = Form([]), source: str = Form('cs_itr'), ipmt: str = Form(''), spdt: str = Form(''), product_model: str = Form(''), year: str = Form(''), group_ids: list[str] = Form([])):
        ...
        if source in {'operations','cs_itr'}:
            from quality_knowledge.scenario_sources import scene_source_records
            rows=scene_source_records(scenario_generation_svc,source,{'product_code':product_code,'ipmt':ipmt,'spdt':spdt,'product_model':product_model,'year':year,'start_month':start_month,'end_month':end_month,'group_ids':group_ids},selected_ids)
